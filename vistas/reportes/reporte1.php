...
<section>
	...
	<tr>
		...
		<td style="width: 75%; font-size: 28px; font-weight: bold;"><span style="margin-left: 1.5%;">Reporte de Tickets Cerrados</span></td>
		<td style="width: 15%; text-align: center;"><b style="font-size: 13px;">'. $fechaLocal . "<br>" . $horaLocal .'</b></td>
	</tr>
	<tr>
		<td style="text-align: left;" colspan="3"><br>Reporte de Tickets cerrados en un rango de fechas</td>
	</tr>
	</table><br>
	...
	';
	$con = new clsConexion();
	$query = "SELECT T.idTicket, C.nombreCompleto, T.asunto, D.fechaCierre, E.nombreCompleto, D.diagnostico FROM ticket as T INNER JOIN cliente as C INNER JOIN diagnostico as D INNER JOIN tecnicos as E WHERE T.idCliente=C.idCliente AND D.idTicket=T.idTicket AND D.idTecnico = E.idTecnico AND T.estado!=0 AND (D.fechaCierre>='". $f1 ."' AND D.fechaCierre<='". $f2 ."') AND D.estadoDiagnostico='Cerrado'";
