...
<section>
	...
	<tr>
		...
		<td style="width: 75%; font-size: 28px; font-weight: bold;"><span style="margin-left: 1.5%;">Reporte de Tickets cerrados por categoría</span></td>
		<td style="width: 15%; text-align: center;"><b style="font-size: 13px;">'. $fechaLocal . "<br>" . $horaLocal .'</b></td>
	</tr>
	<tr>
		<td style="text-align: left;" colspan="3"><br>Reportes de Tickets cerrados según categoría de problema</td>
	</tr>
	</table><br>
	...
	';
	$con = new clsConexion();
	$query = "SELECT T.idTicket, C.nombreCompleto, T.asunto, D.fechaCierre, E.nombreCompleto, U.tipo, U.nombre FROM ticket as T INNER JOIN cliente as C INNER JOIN diagnostico as D INNER JOIN tecnicos as E INNER JOIN categoria as U WHERE T.idCliente=C.idCliente AND D.idTicket=T.idTicket AND D.idTecnico = E.idTecnico AND U.idCategoria = D.idCategoria AND T.estado!=0 AND D.estadoDiagnostico='Cerrado' AND U.idCategoria='". $cat ."'";
	...
	foreach ($contenedor as $fila) {
		...
		$salida.='<td style="text-align: center;">'. date("d/m/Y", strtotime($fila[3])). "<br>". date("h:i A", strtotime($fila[3]))   .'</td>';
		$salida.='<td style="text-align: center;">'. $fila[4] .'</td>';
		$salida.='<td style="text-align: center;">'. $fila[5] .'</td>';
		$salida.='<td style="text-align: center;">'. $fila[6] .'</td>';
		$salida.= '</tr>';
	}
	$salida.= '
	</table>
