...
function eliminarRegistro(id){
	Swal.fire({
        ...
        if (result.value) {
	        var array = id.split(".");
	        id = array[0];
	        id= '../controladores/Departamento.controlador.php?a=elim&idDepartamento=' + id;
	       location.href=id;
        }//termina if
        else{
        }
...
function cargar(){      
    $.ajax({                
        url: 'formDepartamento.php',
        success: function(response){                                    
        	$('#resultado').load('formDepartamento.php');
        }
    });
}
...
function cargar2(id){
    $.ajax({
        url: 'formModificarDepartamento.php',
        type: 'POST',
        dataType: 'html',
        data: {id: id},
    })
